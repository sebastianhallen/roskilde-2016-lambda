...
function saveArtistsToDisk() {
  ...
  .then(acts => {
    return new Promise((resolve, reject) => {
      ...
      const fileStream = fs.writeFile(localData, JSON.stringify(filteredData), {}, resolve);
    })
  })
...
function getActs() { 
  return new Promise((resolve) => {
      fs.access(localData, fs.F_OK, error => {
        if (error) {
          return saveArtistsToDisk().then(() => resolve); 
        }
        resolve();
      });
    })
    .then(() => JSON.parse(fs.readFileSync(localData)))
    .then(data => data.map(artist => new Act(artist)))
    .then(acts => acts.filter(act => act.isPlayingAtFestival))
